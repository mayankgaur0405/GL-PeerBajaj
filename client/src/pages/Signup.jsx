...
export default function Signup() {
  const { startSignup, verifySignupOtp, completeSignup } = useAuth()
  ...
  const update = (k, v) => {
    ...
    if (k === 'email') {
      const validation = validateGLBITMEmail(v)
  ...
  const submit = async (e) => {
    ...
    try {
      if (step === 1) {
        const emailValidation = validateGLBITMEmail(form.email)
        if (!emailValidation.isValid) {
          setEmailError(emailValidation.message)
          return
        }
        await startSignup(form.email)
        setStep(2)
      } else if (step === 2) {
        const res = await verifySignupOtp({ email: form.email, otp })
        setCompleteToken(res.completeToken)
        setStep(3)
      } else if (step === 3) {
        await completeSignup({ completeToken, name: form.name, username: form.username, password: form.password })
        nav('/')
      }
    } catch (e) {
  ...
  return (
    <div className="max-w-md mx-auto rounded-2xl shadow-xl p-6 bg-white text-slate-900 dark:bg-slate-900/80 dark:text-slate-100 backdrop-blur">
      ...
      <form onSubmit={submit} className="space-y-4">
        {step === 1 && (
          <div>
            <label className="block text-sm mb-1 text-slate-700 dark:text-slate-200">College Email</label>
            <input 
        ...
        {step === 2 && (
          <div>
            <label className="block text-sm mb-1 text-slate-700 dark:text-slate-200">Enter OTP</label>
            <input 
              className="w-full rounded-lg px-3 py-2 bg-white dark:bg-slate-800/80 border border-slate-300 dark:border-white/10 focus:outline-none focus:ring-2 focus:ring-blue-500/60 text-slate-900 dark:text-slate-100 placeholder-slate-400 tracking-widest text-center text-lg" 
              placeholder="000000" 
              value={otp} 
              onChange={(e)=>setOtp(e.target.value.replace(/\D/g, '').slice(0,6))} 
            />
            <p className="mt-1 text-xs text-slate-500">We sent a 6-digit code to {form.email}. It expires in 10 minutes.</p>
          </div>
        )}
